...
public class Store {
    ...
    private Map<Category,Map<String,InventoryItem>> aisleInventory; //<category,<productName,InventoryItem>
    ...
    private boolean checkOutCart(Cart cart){
        for(var cartItem:cart.getProducts().entrySet()){
            var inventoryItem=inventory.get(cartItem.getKey());
            int qty=cartItem.getValue();
            if(!inventoryItem.sellItem(qty)) return false;
        }
    ...
    private void listProductsByCategory(boolean includeHeader,boolean includeDetail){
        aisleInventory.keySet().forEach(k->{
                    if(includeHeader) System.out.println("___________\n"+k+"\n___________");
                    if(!includeDetail){
